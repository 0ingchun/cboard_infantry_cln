...
/**
  ...
  * @retval         无
  */
__NO_RETURN void INS_task(__attribute__((unused)) void *argument)
{
    delay_init();

    TASK_CTRL_INIT(SELF, INS_TASK_INIT_TIME, 1, ENABLE);
    ...
    get_angle(INS_quat, SELF.INS_angle + INS_YAW_ADDRESS_OFFSET, SELF.INS_angle + INS_PITCH_ADDRESS_OFFSET, SELF.INS_angle + INS_ROLL_ADDRESS_OFFSET);
...
static void imu_temp_control(fp32 temp)
{
    ...
    {
        //在没有达到设置的温度，一直最大功率加热
        if (temp > (fp32)get_control_temperature())
        {
            temp_constant_time++;
...
{
    if (gyro_offset == NULL || gyro == NULL || offset_time_count == NULL) return;
...
//        imu_angle_go = __HAL_DMA_CLEAR_FLAG(hspi1.hdmarx, __HAL_DMA_GET_TC_FLAG_INDEX(hspi1.hdmarx)); //我拉的屎
